...
onMounted(() => {
    // 获取用户信息 - 从localStorage中获取用户ID
    // const address = localStorage.getItem("accountAddress") || "";
    // 先获取用户ID
    patientId.value = parseInt(localStorage.getItem("userId") || "0");
    
    // 如果有userId则直接获取信息，否则通过地址查询
    if (patientId.value) {
        getUserInfo();
    } else {
...
const getUserInfo = async () => {
    try {
        const response = await getUserDetails(patientId.value);
